...
function getFavsByCoords(){	
	...
	try {  	
		...
		var message = new jsonrpcmsg('getFavsByCoord',
					[new jsonrpcval(latitude, 'double'),new jsonrpcval(longitude, 'double'),new jsonrpcval(art, 'string'),new jsonrpcval(umkreis, 'string'), new jsonrpcval(sortieren, 'string'), new jsonrpcval(localStorage["favorits"], 'string')]
					,null
		);
...
function changeFavorit(viewId, primaryId, naming) {
	...
	debugger;
	/* ID speichern */
	if(localStorage.getItem(primaryId) == null && primaryId != null) {
		localStorage.setItem(primaryId, primaryId);
		var favs;
		if(localStorage["favorits"]) {
			favs = JSON.parse(localStorage["favorits"]);
		}
		if(favs == null) {
			var favs = [];
			favs[0] = primaryId;
		} else {
			if(jQuery.inArray(primaryId, favs) == -1) {
				favs.push(primaryId);
			}
		}
		localStorage["favorits"] = JSON.stringify(favs);

		/*
		localStorage.setItem(primaryId, primaryId);
		var favs = localStorage.getItem("favList");
		localStorage.setItem("favList", favs + " " + primaryId + ";");
		alert("Favoritenlisten : " + localStorage["favorits"]);*/
	}
	else 
	{   /* ID löschen */
		...
		if(favs == null) {
			...
			debugger;
			var indexToRemove = jQuery.inArray(primaryId, favs);
			if(indexToRemove != -1) {
