   ...
   protected function DataByCoords() {
        switch($this->method) {
			case 'GET':
				...
				break;
			case 'POST':
				$data = (object)  json_decode(file_get_contents('php://input'), false);
				if(!empty($data->latitude) 
						&& !empty($data->longitude) 
							&& !empty($data->article)
								&& !empty($data->distance)
									&& !empty($data->sortBy)){					
					return array('stationList' => $this->GetDataByCoords($data->article, $data->distance,$data->sortBy, new location($data->latitude,$data->longitude)));
				} else {
					return array('Missing or Invalid Arguments', 400);
				}
				break;				
