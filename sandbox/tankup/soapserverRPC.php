...
function getCoordsByTownRPC(GetCoordsByTownRequest $arg1){
	// Logging class initialization

	if($log == null) $log = new Logging();	 
	$log->lfile('log_debug.txt');
	$log->lwrite("Aufruf getCoordsByTown SOAPSERVERRPC mit ..." );
	$log->lwrite(" :" . $arg1);
	$log->lclose();
...
function getTownByCoordsRPC($lat, $lng)
{
	$town = "blub"; // TODO Fehler 
	$db_id = mysql_connect("localhost","web1162","bX2KARTc");
    if(!$db_id)
    {	
        die("Verbindungsaufbau ist gescheitert");
	...
	{		
		...
		$arg1 = $lng;
			$log = new Logging();   			
	$log->lfile('log_debug.txt');
	$log->lwrite("SoapServerRPC:getTownByCoordsRPC " . $arg2);
	$log->lclose();	
	}
	catch (SoapFault $fault) {
...
$server->addFunction("getCoordsByTownRPC");
$server->addFunction("getTownByCoordsRPC");
$server->addFunction("getDataByCoordsRPC");
$server->handle();
