        ...
        switch($this->method) {
			case 'GET':
				if(!empty($_GET['latitude']) && !empty($_GET['longitude'])){					
					return $this->GetTownByCoords(new location($_GET['latitude'],$_GET['longitude']));
				} else {
					return array('Missing or Invalid Arguments', 400);
				}
				break;
				...
				if(!empty($_GET['latitude']) 
						&& !empty($_GET['longitude']) 
							&& !empty($_GET['article'])
								&& !empty($_GET['distance'])
									&& !empty($_GET['sortBy'])){					
					return $this->GetDataByCoords($_GET['article'], $_GET['distance'],$_GET['sortBy'], new location($_GET['latitude'],$_GET['longitude']));
				} else {
					return array('Missing or Invalid Arguments', 400);
				}
				break;
