...
$getDataByCoord_doc = 'Get Data by Location (longitude and latitude).';
/*
function getFavsByCoord($params)
{
	...
	{ 
		...
		$favsId = explode (";",$favsList);
        for($i=0; $i<$favsId->arraysize(); $i++)
        {
            $str=$favsId[$i];
				if($log == null) $log = new Logging();	 
				$log->lfile('log_info.txt');
				$log->lwrite("GetFavs mit id: ". $str);
				$log->lclose();            
        }
	}
	catch (Exception $fault) {
	...
	if($log == null) $log = new Logging();	 
	$log->lfile('log_info.txt');
	$log->lwrite("Aufruf getFavsByCoord mit Artikel ".$art." , Umkreis: ".$umkreis." , Sortieren nach: ".$sortieren." , Long: ".$lon." , Lat: ".$lat.".");
	$log->lclose();	 
...
$getFavsByCoord_sig = array(array('struct', 'double','double','string','string','string', 'string')));
$getFavsByCoord_doc = 'Get Favs by Location (longitude and latitude).'; */
...
$server = new jsonrpc_server(array(
                        ...
                        array('function' => 'getCoordByTown',
                              ...
                              'docstring' => $getDataByCoord_doc),/*
								'getFavsByCoord' =>							  
								array('function' => 'getFavsByCoord',
                              'signature' => $getFavsByCoord_sig,
                              'docstring' => $getFavsByCoord_doc)*/
                        )
);
