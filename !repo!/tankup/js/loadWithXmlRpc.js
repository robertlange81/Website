...
var xmlRpcServer = 'www.sparesprit.de';
var xmlRpcPath = 'xmlrpcserver.php';
...
function getCoordsByTown() {	
	...
	try {  
		...
		var client = new xmlrpc_client(xmlRpcPath,xmlRpcServer,80); 
		var message = new xmlrpcmsg('getCoordsByTown',	[new xmlrpcval(town, 'string')], 1);                     
...
function getTownByCoords(latitude, longitude) {
	...
	try {  
		var client = new xmlrpc_client(xmlRpcPath,xmlRpcServer,80); 
		var message = new xmlrpcmsg('getTownByCoords',	[new xmlrpcval(latitude, 'double'),new xmlrpcval(longitude, 'double')], 1);                     
		var result = client.send(message, 1000, function(msgResp){
			var struct = msgResp.value();
			//console.log(msgResp.serialize());
...
function getDataByCoords(latitude, longitude){	
	...
	try {  	
		...
		var client = new xmlrpc_client(xmlRpcPath,xmlRpcServer,80); 
		var message = new xmlrpcmsg('getDataByCoords',
					[new xmlrpcval(latitude, 'double'),new xmlrpcval(longitude, 'double'),new xmlrpcval(art, 'string'),new xmlrpcval(umkreis, 'string'), new xmlrpcval(sortieren, 'string')], 
					1
		);
		
		client.request_charset_encoding="UTF-8";                     
		client.send(message, 1000, function(msgResp){
